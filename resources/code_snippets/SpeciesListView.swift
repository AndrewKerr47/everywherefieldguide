...
//   - Panel shows three stacked filter options, each with an independent
//     species count (how many species that source alone would return)
...
struct SpeciesListView: View {
    ...
    private let filterDefs: [(id: String, label: String)] = [
        ("CN-1985",   "Cunha et al. 1985"),
        ("UFSC-2023", "CNF Herpetofauna Survey 2015–2023"),
        ("inat",      "iNaturalist"),
    ]

    // ── Species count per filter (independent — ignores other active filters) ─
    ...
    // ── Filtered species (survey filter + text search composed) ───────────────
                    ...
                    if filteredSpecies.isEmpty {
                        ...
                        ForEach(filteredSpecies) { species in
                            NavigationLink(
                                ...
                                SpeciesRowView(species: species, showPortugueseNames: showPortugueseNames)
                            }
                            .buttonStyle(.plain)
                        }
                    }
...
struct FilterPanelView: View {
    ...
    let filteredCount: Int   // actual deduplicated count from parent
                    ...
                    HStack(alignment: .bottom) {
                        ...
                        Button {
                            ...
                            Text("🇧🇷")
                                ...
                                .background(
                                    Circle()
                                        .fill(.white.opacity(showPortugueseNames ? 0.25 : 0.12))
                                )
                                .clipShape(Circle())
                                .opacity(showPortugueseNames ? 1.0 : 0.6)
                        }
                        .buttonStyle(.plain)
                    }
                    .padding(.top, 6)

...
struct SpeciesListView: View {
    ...
    private var filterDefs: [(id: String, label: String)] {
        var defs: [(id: String, label: String)] = [
            ...
            ("inat",      "iNaturalist"),
            ("seen", LocaleManager.shared.localizedString("filter.seen", defaultValue: "Seen")),
        ]
        #if DEBUG
    ...
    private var emptyState: some View {
        VStack(spacing: 10) {
            ...
            Text("list.empty.title", comment: "Empty state heading in species list")
                .font(.bodyMedium)
                .foregroundColor(.onSurfaceVariant)
            Text("list.empty.subtitle", comment: "Empty state subtext in species list")
                .font(.sectionLabel)
                .textCase(.uppercase)
                .tracking(1.0)
...
struct FilterPanelView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack {
                Text("filter.title", comment: "Section label above the filter checkboxes")
                    .font(.sectionLabel)
                    .textCase(.uppercase)
                    .tracking(1.0)
                    .foregroundColor(.outline)
...
struct BannerView: View {
    ...
    var subtitleText: String {
        if isFiltered {
            let fmt = LocaleManager.shared.localizedString("banner.subtitle_filtered", defaultValue: "%1$lld OF %2$lld SPECIES · SURVEY-CONFIRMED")
            return fmt
                .replacingOccurrences(of: "%1$lld", with: "\(filteredCount)")
                .replacingOccurrences(of: "%2$lld", with: "\(totalCount)")
        } else {
            let fmt = LocaleManager.shared.localizedString("banner.subtitle_all", defaultValue: "%lld SPECIES · SURVEY-CONFIRMED")
            return String(format: fmt, totalCount)
        }
    }
        ...
        VStack(spacing: 0) {
            ...
            ZStack(alignment: .bottomLeading) {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    Text("banner.snakes_of", comment: "First line of the banner title block")
                    ...
                    HStack(alignment: .center) {
                        ...
                        Button {
                            ...
                            Text("banner.local_name", comment: "Toggle button to show local common names")
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            TextField(String(localized: "search.placeholder",
                             ...
                             bundle: .main,
                             locale: LocaleManager.shared.current),
                      text: $text)
